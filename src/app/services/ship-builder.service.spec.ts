...
import { HullIntegrity } from '../models/HullIntegrity';
import { SHIP_TYPES } from '../models/ShipTypes';
...
describe('ShipBuilderService', () => {
  ...
  describe('build', () => {
    ...
    it('should create destroyer with hull integrity intact', () => {
      ...
      expect(result.hullStructure[0].hullIntegrity).toBe(HullIntegrity.Intact);
    });

    it('should create destroyer with hull correct health value', () => {
      const result = instance.build(SHIP_TYPES.destroyer);

      expect(result.hullStructure[0].healthValue).toBe(1);
    });

    it('should create destroyer with hull location default', () => {
      const result = instance.build(SHIP_TYPES.destroyer);

      expect(result.hullStructure[0].Location.x).toBe(0);
      expect(result.hullStructure[0].Location.y).toBe(0);
    });
